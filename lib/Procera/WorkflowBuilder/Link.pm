package Procera::WorkflowBuilder::Link;
...
sub validate {
    ...
    # if dest operation is parallel by dest_property, make sure that
    # source_property is_many
...
sub _validate_general_operation_type {
    ...
    if (defined($self->$property_name)) {
        unless ($self->$property_name->does(
                'Procera::WorkflowBuilder::Detail::Operation')) {
            die sprintf(
                "Expected %s => Procera::WorkflowBuilder::Detail::Operation, "
