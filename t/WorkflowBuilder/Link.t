...
use_ok('Procera::WorkflowBuilder::Link');

subtest 'Typical Link' => sub {
    my $source_op = Procera::WorkflowBuilder::Command->new(
        name => 'source op',
        command => 'WorkflowBuilder::Test::DummyCommand'
    );
    my $destination_op = Procera::WorkflowBuilder::Command->new(
        name => 'destination op',
        command => 'WorkflowBuilder::Test::DummyCommand'
    );

    my $link = Procera::WorkflowBuilder::Link->new(
...
subtest 'Valid Operation Type for Notify' => sub {
    dies_ok { Procera::WorkflowBuilder::Link->new(
            source => 'INVALID_OPERATION', source_property => 'foo',
            destination_property => 'bar') }
        'invalid source operation dies';
...
subtest 'Parallel-By Link Plus Single Link' => sub {
    ...
    my $destination_op = Procera::WorkflowBuilder::Command->new(
        name => 'destination op',
        command => 'WorkflowBuilder::Test::DummyCommandTwoInputs',
        parallel_by => 'input',
    );

    my $link_many = Procera::WorkflowBuilder::Link->new(
        source => $source_op, source_property => 'many_output',
        destination => $destination_op, destination_property => 'input'
    );
    ok($link_many->validate, "parallelBy link validates");

    my $link_single = Procera::WorkflowBuilder::Link->new(
        source => $source_op, source_property => 'single_output',
